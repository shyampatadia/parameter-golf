...
\textbf{Quantization:} store each weight in fewer bits (INT8 = 8-bit integers, INT6 = 6-bit). \\
\textbf{Intuition:} at 16~MB, INT8 fits $\sim$8M params, INT6 fits $\sim$11M.
\emph{Should} be a free win.
...
\vspace{0.4em}
\textcolor{accent}{\textbf{Surprise.}} \emph{QAT} (quantization-aware training:
simulate the rounding during training) matches the loss in memory, but uniform
6-bit bins can't represent the trained weights at this scale.\\[0.4em]
...
{\scriptsize \textbf{Position:} how the model knows token order. RoPE rotates
each query/key by an angle that depends on its position; \emph{partial} RoPE
applies it to only the first 32 of 64 head dimensions.\\
Partial RoPE \good{1.215\bpb} -- best in-budget.}

\column{0.5\textwidth}
\centering
\includegraphics[width=\linewidth]{curves_o3_optim.pdf}\\
{\scriptsize \textbf{Optimizer:} Muon orthogonalizes the gradient of each
weight matrix before stepping (Newton--Schulz iteration). \\
Muon \good{1.217\bpb} $\ll$ AdamW (1.44), Adafactor (1.49). \emph{Load-bearing.}}
\end{columns}

\vspace{0.3em}
\hrule
\vspace{0.2em}
{\small \textbf{Weight factorizations} (low-rank $W{=}UV^\top$, block-diagonal,
fixed random projection): force structure on the weights to save parameters. \\
...
\footnotesize TurboQuant: rotate each weight row by a random orthogonal
matrix, then quantize with a Lloyd--Max codebook tuned for the resulting
distribution. Originally an \emph{inference-time} KV-cache method
(Zandieh~2024); we apply it to weights at 4 bits.\\
$\rightarrow$ \bad{3.60\bpb} final. Catastrophic.

\vspace{0.5em}
\normalsize\textbf{(b) Same quantizer, ease arch:}\\
INT8 $+$ 11~layers $+$ MLP$=$2.\\
$\rightarrow$ \good{1.194\bpb}, $-$0.030 below reference.

\vspace{0.6em}
\hil{Verdict: the quantizer was paying the bill.}\\
{\footnotesize The architecture lever works -- as long as you don't crush precision to fund it.}
\end{columns}
\end{frame}
...
{\scriptsize Share \emph{one} K/V projection across all heads instead of one per
head. Frees parameters that fund a 12-layer model.}\\
...
{\scriptsize Per-head learnable scalar on the query projection.}\\
$g_q^{(0)} \in \{1.5, 3.0, 5.0\}$. Spread \alert{0.002\bpb}. \\
\hil{Null effect.} Learned $g_q$ converges regardless.\\[0.4em]

\textbf{O8 -- LR / warmup sweep ($3{\times}3$).}\\
Spread \alert{0.0019\bpb}. \\
\hil{Null effect.} Default recipe is at the optimum.\\[0.4em]

\textbf{O9 -- Eval protocol.}\\
{\scriptsize Sliding-window eval: instead of scoring non-overlapping
1024-token chunks, slide the window 256 tokens at a time so every token sees
more left-context.}\\
Same weights $\rightarrow$ \hil{1.160\bpb}. \textit{Not leaderboard-comparable, but a fair upper bound.}
\end{columns}
...
\vspace{-0.2em}
{\scriptsize \textbf{Pareto frontier} = the lower-left envelope: each point on it is the best bpb
\emph{achievable} at that size.}\\[0.2em]
